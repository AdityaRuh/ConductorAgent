...
Poll Linear for "Require Changes" tickets. Identify associated PRs. Resolve code review comments with minimal, validated changes. Create PRs when needed. Reply with proof. Track everything. Move tickets to "Code Review" when done.
...
### Trigger
- **Automatic:** Cron every 30 minutes polls Linear for "Require Changes" tickets
...
```
Linear ticket in "Require Changes" detected
    ↓
1. POLL LINEAR TICKETS (every 30 min, FREE)
    - Fetch all tickets in "Require Changes" state
...
20. UPDATE LINEAR TICKET (FREE)
    Move ticket state: Require Changes → Code Review
    Add comment:
