...
> Every 30 minutes, poll Linear for "Require Changes" tickets and process associated PRs.

## Checklist (in order)

### 1. Poll Linear Tickets
```bash
# Fetch all tickets in "Require Changes" state with linked PRs
linear_get_issues_by_state "Require Changes"
...
- Move state: `Require Changes` → `Code Review`
