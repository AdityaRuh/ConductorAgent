...
PR Resolver is a ticket-driven agent built on [OpenClaw](https://openclaw.ai). It polls **Linear** for tickets in "Require Changes", finds their associated GitHub PRs, and processes every unresolved review comment — automatically.
...
```
Linear "Require Changes" tickets  (every 30 min)
            ↓
...
| Feature | Detail |
|---------|--------|
| **Ticket-driven** | Linear "Require Changes" is the trigger — not random GitHub polling |
